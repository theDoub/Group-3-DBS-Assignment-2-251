...
BEGIN
    ...
    DECLARE v_PriceAtPurchase DECIMAL(10, 2); -- PriceAtPurchase before apply discount
    DECLARE v_BasePrice DECIMAL(10, 2); -- BasePrice = Quantity * PricePerItem
    DECLARE v_ReducedPrice DECIMAL(10, 2); -- E.g. price 30, discount 10% -> ReducedPrice = 30*10% = 3
    DECLARE v_NewPrice DECIMAL(10, 2); -- NewPrice = PriceAtPurchase - ReducedPrice
    ...
    SELECT PricePerItem, Quantity, PriceAtPurchase
    INTO v_PricePerItem, v_Quantity, v_PriceAtPurchase
    FROM OrderItem
    ...
    IF v_DiscountType = 'percentage' THEN
        SET v_ReducedPrice = v_BasePrice * (v_DiscountValue / 100);
    ELSEIF v_DiscountType = 'fixed_amount' THEN
        SET v_ReducedPrice = v_DiscountValue;
    ELSE
        SET v_ReducedPrice = 0.0;
    END IF;

    -- 7. Update giá
    SET v_NewPrice = GREATEST(v_PriceAtPurchase - v_ReducedPrice, 0);
...
END //
-- ------------------------------
CREATE PROCEDURE UnapplyDiscountLogic (
    IN p_DiscountID INT,
    IN p_OrderID VARCHAR(10),
    IN p_OrderNo INT
)
BEGIN
    ...
    -- 7. Update giá
    SET v_NewPrice = v_PriceAtPurchase + v_ReducedPrice; -- This is the only difference from ApplyDiscountLogic
...
END //
-- ---------------------------------
-- Check if the conditions (Validity Period, MaxAppliedItem) are fullfiled
...
-- CREATE TRIGGER trg_Discountapply_Delete
-- AFTER DELETE ON DiscountApply
-- FOR EACH ROW
-- BEGIN
-- 	CALL UnapplyDiscountLogic(
--         OLD.DiscountID, 
--         OLD.OrderID, 
--         OLD.OrderNo
--     );
