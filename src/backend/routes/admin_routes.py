...
def delete_book(book_id):
    ...
    print(f"[delete_book] Attempting to delete book {book_id}")
    ...
    try:
        ...
        print(f"[delete_book] Found {len(editions)} editions for book {book_id}")
        ...
        # Delete all BookAuthor relations
        delete_book_author_sql = "DELETE FROM BookAuthor WHERE BookID = %s"
        cursor.execute(delete_book_author_sql, (book_id,))
        
        # Delete all BookCategory relations
        delete_book_category_sql = "DELETE FROM BookCategory WHERE BookID = %s"
        cursor.execute(delete_book_category_sql, (book_id,))
...
def create_author():
    ...
    sql = "INSERT INTO Author (Name, Biography, Nationality, ImageURL) VALUES (%s, %s, %s, %s)"
    
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(sql, (data.get('Name'), data.get('Biography'), data.get('Nationality'), data.get('ImageURL')))
        conn.commit()
        cursor.close()
        conn.close()
...
def update_author(author_id):
    ...
    sql = "UPDATE Author SET Name = %s, Biography = %s, Nationality = %s, ImageURL = %s WHERE AuthorID = %s"
    
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(sql, (data.get('Name'), data.get('Biography'), data.get('Nationality'), data.get('ImageURL'), author_id))
        conn.commit()
        cursor.close()
        conn.close()
...
def approve_payment(payment_id):
    ...
    try:
        ...
        cursor.execute(
         ...
         # If order has printed books, create delivery record (only if not already created)
        if has_printed:
            cursor.execute("SELECT DeliveryID FROM Delivery WHERE OrderID = %s LIMIT 1", [order_id])
            ...
            if not existing_delivery:
                ...
                print(f"[approve_payment] Delivery already exists for Order {order_id}, skipping insert")
