...
function showOrderDetail(orderId) {
    ...
    .then(order => {
        ...
        // Fetch reviewable items to check payment status and review status
        fetch(`http://127.0.0.1:5000/api/reviews/order/${orderId}/items`)
        .then(res => res.json())
        .then(reviewableItems => {
            const tbody = document.getElementById("itemsTable");
            tbody.innerHTML = order.items.map(item => {
                console.log("Order Item:", item);
                const hasDiscount = item.AppliedDiscounts && item.AppliedDiscounts.trim().length > 0;
                const reviewItem = reviewableItems.find(ri => ri.OrderNo === item.OrderNo);
                const paymentCompleted = reviewItem && reviewItem.PaymentStatus === 'completed';
                const alreadyReviewed = reviewItem && reviewItem.AlreadyReviewed !== null;
                ...
                if (order.Status === 'pending') {
                    ...
                    if (alreadyReviewed) {
                        actionBtn = `<span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>Reviewed</span>`;
                    } else if (paymentCompleted) {
                        actionBtn = `<button onclick="openReviewModal(${item.OrderNo}, '${order.OrderID}', '${item.BookTitle.replace(/'/g, "\\'")}', '${item.FormatType}', 'completed', false)" class="btn btn-sm btn-warning"><i class="bi bi-star me-1"></i>Review</button>`;
                    } else {
                        actionBtn = `<small class="text-muted">Payment pending</small>`;
                    }
                } else {
                ...
                return `
                    <tr>
                        ...
                        <td>${hasDiscount ? `<span class="badge" style="background-color: var(--primary); color: white;"><i class="bi bi-tag-fill me-1"></i>${item.AppliedDiscounts}</span>` : '<small class="text-muted">-</small>'}</td>
                        <td>${actionBtn}</td>
                    </tr>
                `;
            }).join('');
        ...
        .catch(err => {
            ...
            // Fallback to original display without review buttons
            const tbody = document.getElementById("itemsTable");
            tbody.innerHTML = order.items.map(item => {
                const hasDiscount = item.AppliedDiscounts && item.AppliedDiscounts.trim().length > 0;
                let actionBtn = '';
    ...
    .then(discounts => {
        ...
        processed.forEach((d, index) => {
            ...
            if (index === 0 && d.isValid) {
                badge = `<span class="badge" style="background-color: var(--accent-1); color: white; margin-bottom: 0.5rem;"><i class="bi bi-star-fill me-1"></i>Recommended</span>`;
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (reviewModalEl) {
        reviewModalObj = new bootstrap.Modal(reviewModalEl);
...
function openReviewModal(orderNo, orderId, bookTitle, formatType, paymentStatus, alreadyReviewed) {
    if (alreadyReviewed) {
        Swal.fire('Already Reviewed', 'You have already submitted a review for this item.', 'info');
        return;
    }
    
    if (paymentStatus !== 'completed') {
        Swal.fire('Payment Required', 'You can only review items after payment is completed.', 'warning');
        return;
    }
    
    currentReviewItem = { orderNo, orderId, bookTitle, formatType };
    selectedRating = 0;
    
    document.getElementById('reviewItemInfo').innerHTML = `
        <strong>${bookTitle}</strong><br>
        <small class="text-muted">Format: ${formatType}</small>
    `;
...
function submitReview() {
    ...
    const reviewData = {
        ...
        orderNo: currentReviewItem.orderNo,
    ...
    .then(data => {
        if (data.error) {
            ...
            showOrderDetail(currentOrderId); // Refresh to show review button state
        }
    })
    .catch(err => {
