...
function displayAuthors(authors, containerId) {
    ...
    authors.forEach(author => {
        ...
        // Don't use ImageURL from API, use local images instead
        const avatarContent = `<img src="images/authors/${author.Name.toLowerCase().replace(/ /g, '-').replace(/\./g, '')}.jpg" alt="${author.Name}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
        if (!avatarContent) {
