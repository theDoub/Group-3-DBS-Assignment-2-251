...
def add_to_cart():
    ...
    admin_check_sql = "SELECT AccountType FROM Account WHERE AccountID = %s"
    account = query_one(admin_check_sql, [account_id])
    
    if account and account['AccountType'] == 'Administrator':
        return jsonify({"error": "Administrators cannot add items to cart. Please use a customer account."}), 403
