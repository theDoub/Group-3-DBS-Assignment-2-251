...
def list_authors():
    ...
    sql = """
        ...
        GROUP BY A.AuthorID, A.Name, A.Biography, A.Nationality, A.ImageURL
        ORDER BY A.Name ASC
    """
...
def get_top_selling_authors():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as ConfirmedOrders FROM `Order` WHERE Status = 'confirmed'")
        confirmed_result = cursor.fetchone()
        confirmed_count = confirmed_result['ConfirmedOrders'] if confirmed_result else 0
        print(f"[get_top_selling_authors] Total confirmed orders: {confirmed_count}")
        
        # DEBUG: Check all orders
        cursor.execute("SELECT COUNT(*) as TotalOrders FROM `Order`")
        total_result = cursor.fetchone()
        total_count = total_result['TotalOrders'] if total_result else 0
        print(f"[get_top_selling_authors] Total orders (all statuses): {total_count}")
        
        cursor.callproc('GetTopSellingAuthors', [1])
...
def list_publishers():
    ...
    sql = """
        SELECT
            ...
            ) FROM Contact C WHERE C.PublisherID = P.PublisherID) AS Contacts,
