...
class="btn btn-sm mb-2 w-100 d-block format-badge" style="border-color: var(--primary); color: var(--primary);">
        ...
        fetch(`http://127.0.0.1:5000/api/reviews/book/${b.BookID}/average`)
            .then(r => r.json())
            .then(ratingData => {
                const avgRating = ratingData.AverageRating || 0;
                const totalReviews = ratingData.TotalReviews || 0;
            ...
            .catch(err => {
                ...
                // Display without rating if fetch fails
                container.innerHTML += `
                    <div class="col">
                        <div class="card h-100 book-card">
                            <img src="${imgUrl}" class="card-img-top book-cover" alt="${b.Title}" style="cursor: pointer;"
...
function buy(formatId, title, type) {
    ...
    Swal.fire({
        title: `Add to Cart`,
        html: `Book: <b>${title}</b><br>Format: <span class="badge bg-info text-dark">${type}</span>`,
        input: 'number',
        inputValue: 1,
        inputLabel: 'Quantity',
            ...
            paymentList.innerHTML = data.map(payment => `
                <div class="card border-1 rounded-3" style="background: rgba(13,110,253,0.05);">
                    <div class="card-body">
                        ...
                        <button onclick="approvePayment('${payment.PaymentID}', '${payment.OrderID}')" class="btn btn-sm w-100" style="background: linear-gradient(125deg, var(--primary) 50%, var(--accent) 100%); color: white; border: none;">
            ...
            deliveryList.innerHTML = data.map(delivery => `
                <div class="card border-1 rounded-3" style="background: rgba(13,110,253,0.05);">
                    <div class="card-body">
                        ...
                        <button onclick="updateDelivery('${delivery.DeliveryID}', '${delivery.OrderID}')" class="btn btn-sm w-100" style="background: linear-gradient(125deg, var(--primary) 50%, var(--accent) 100%); color: white; border: none;">
